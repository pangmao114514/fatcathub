...
local function flingAll()
    ...
    local originalPos = localRoot.Position
        ...
        if player ~= localPlayer then
            ...
            if targetChar and targetChar:FindFirstChild("HumanoidRootPart") then
                ...
                if targetHumanoid and targetHumanoid.Health > 0 then
                    -- 切换到目标位置
                    localRoot.CFrame = targetRoot.CFrame * CFrame.new(0, 1.5, 0)
                    task.wait()
                    
                    -- 给目标施加超高速度（X和Z方向随机，Y方向向上）
                    local xForce = math.random(-5000, 5000)
                    local zForce = math.random(-5000, 5000)
                    targetRoot.Velocity = Vector3.new(xForce, 3000, zForce)
                    targetRoot.RotVelocity = Vector3.new(900000, 900000, 900000)
                    
                    -- 给自己也加一点速度防止卡住
                    localRoot.Velocity = Vector3.new(xForce / 2, 1500, zForce / 2)
                    
                    task.wait(0.05)
                    
                    -- 再补一次力确保甩飞效果
                    targetRoot.Velocity = Vector3.new(xForce * 1.5, 4000, zForce * 1.5)
                end
            end
        end
    end
