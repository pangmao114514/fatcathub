...
local function flingAll()
    ...
    if not localChar then flingRunning = false return end
    local localRoot = localChar:FindFirstChild("HumanoidRootPart")
    if not localRoot then flingRunning = false return end
            ...
            if targetChar and targetChar:FindFirstChild("HumanoidRootPart") then
                ...
                if targetHumanoid and targetHumanoid.Health > 0 then
                    ...
                    for i = 1, 30 do
                        ...
                        local yOffset = math.sin(angle * 2) * 1.5
                        
                        localRoot.CFrame = targetRoot.CFrame * CFrame.new(xOffset, yOffset + 1.5, zOffset)
                        task.wait()
                        
                        targetRoot.Velocity = Vector3.new(xOffset * 2000, 3000 + yOffset * 500, zOffset * 2000)
                        targetRoot.RotVelocity = Vector3.new(5000, 5000, 5000)
                        
                        if i % 5 == 0 then
                            localRoot.Velocity = Vector3.new(xOffset * 500, 1000, zOffset * 500)
                        end
                    end
                    
                    targetRoot.Velocity = Vector3.new(math.random(-8000, 8000), 5000, math.random(-8000, 8000))
                    targetRoot.RotVelocity = Vector3.new(10000, 10000, 10000)
                    task.wait(0.1)
                end
            end
        end
